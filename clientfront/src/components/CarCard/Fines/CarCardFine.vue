        ...
        <v-card class="mb-3">
            <v-card-media>
                <v-container class="py-2">
                    <v-layout align-center row fill-height justify-content-start>
                        <v-flex text-xs-left>
                            <p class="subheading my-0">Штрафы</p>                            
                        </v-flex>
                        <v-flex>
                            <v-checkbox label="Не оплаченные" v-model="onlyOwed" hide-details></v-checkbox>
                        </v-flex>
...
export default {
    ...
    computed: {
        filteredFines() {
            if(this.onlyOwed) 
                return this.items.filter(fine => fine.paid === 0);
            else
                return this.items;
        }
    },
